...
public class User32 {
	...
	public static int UnregisterClass(CharSequence lpClassName, long hInstance) {
		...
		try {
			ByteBuffer lpClassNameEncoded = memEncodeUTF16(lpClassName, true, stack);
	...
	public static long CreateWindowEx(int dwExStyle, CharSequence lpClassName, CharSequence lpWindowName, int dwStyle, int x, int y, int nWidth, int nHeight, long hWndParent, long hMenu, long hInstance, long lpParam) {
		...
		try {
			ByteBuffer lpClassNameEncoded = memEncodeUTF16(lpClassName, true, stack);
			ByteBuffer lpWindowNameEncoded = memEncodeUTF16(lpWindowName, true, stack);
			return nCreateWindowEx(dwExStyle, memAddress(lpClassNameEncoded), memAddress(lpWindowNameEncoded), dwStyle, x, y, nWidth, nHeight, hWndParent, hMenu, hInstance, lpParam);
		} finally {
			stack.setPointer(stackPointer);
		}
	}
	...
	public static int SetWindowText(long hWnd, CharSequence lpString) {
		MemoryStack stack = stackGet(); int stackPointer = stack.getPointer();
		try {
			ByteBuffer lpStringEncoded = memEncodeUTF16(lpString, true, stack);
	...
	public static long LoadIcon(long instance, CharSequence iconName) {
		MemoryStack stack = stackGet(); int stackPointer = stack.getPointer();
		try {
			ByteBuffer iconNameEncoded = memEncodeUTF16(iconName, true, stack);
	...
	public static long LoadCursor(long instance, CharSequence cursorName) {
		MemoryStack stack = stackGet(); int stackPointer = stack.getPointer();
		try {
			ByteBuffer cursorNameEncoded = memEncodeUTF16(cursorName, true, stack);
	...
	public static int EnumDisplayDevices(CharSequence lpDevice, int iDevNum, DISPLAY_DEVICE lpDisplayDevice, int dwFlags) {
		MemoryStack stack = stackGet(); int stackPointer = stack.getPointer();
		try {
			ByteBuffer lpDeviceEncoded = lpDevice == null ? null : memEncodeUTF16(lpDevice, true, stack);
	...
	public static int EnumDisplaySettingsEx(CharSequence lpszDeviceName, int iModeNum, DEVMODE lpDevMode, int dwFlags) {
		MemoryStack stack = stackGet(); int stackPointer = stack.getPointer();
		try {
			ByteBuffer lpszDeviceNameEncoded = lpszDeviceName == null ? null : memEncodeUTF16(lpszDeviceName, true, stack);
