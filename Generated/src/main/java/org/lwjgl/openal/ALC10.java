...
public class ALC10 {
	...
	public static long alcOpenDevice(CharSequence deviceSpecifier) {
		...
		try {
			ByteBuffer deviceSpecifierEncoded = deviceSpecifier == null ? null : memEncodeUTF8(deviceSpecifier, true, stack);
	...
	public static boolean alcIsExtensionPresent(long deviceHandle, CharSequence extName) {
		MemoryStack stack = stackGet(); int stackPointer = stack.getPointer();
		try {
			ByteBuffer extNameEncoded = memEncodeASCII(extName, true, stack);
	...
	public static int alcGetEnumValue(long deviceHandle, CharSequence enumName) {
		MemoryStack stack = stackGet(); int stackPointer = stack.getPointer();
		try {
			ByteBuffer enumNameEncoded = memEncodeASCII(enumName, true, stack);
