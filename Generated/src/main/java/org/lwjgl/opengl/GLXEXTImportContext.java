...
public class GLXEXTImportContext {
	...
	public static long glXGetCurrentDisplayEXT() {
		long __functionAddress = GL.getCapabilities().glXGetCurrentDisplayEXT;
		if ( CHECKS )
	...
	public static int nglXQueryContextInfoEXT(long display, long context, int attribute, long value) {
		long __functionAddress = GL.getCapabilities().glXQueryContextInfoEXT;
		if ( CHECKS ) {
			checkFunctionAddress(__functionAddress);
	...
	public static long glXGetContextIDEXT(long context) {
		long __functionAddress = GL.getCapabilities().glXGetContextIDEXT;
		if ( CHECKS ) {
			checkFunctionAddress(__functionAddress);
			checkPointer(context);
		}
	...
	public static long glXImportContextEXT(long display, long contextID) {
		long __functionAddress = GL.getCapabilities().glXImportContextEXT;
		if ( CHECKS ) {
			checkFunctionAddress(__functionAddress);
			checkPointer(display);
		}
	...
	public static void glXFreeContextEXT(long display, long context) {
		long __functionAddress = GL.getCapabilities().glXFreeContextEXT;
		if ( CHECKS ) {
			checkFunctionAddress(__functionAddress);
			checkPointer(display);
			checkPointer(context);
