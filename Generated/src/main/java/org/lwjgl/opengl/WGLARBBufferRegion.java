...
public class WGLARBBufferRegion {
	...
	public static long wglCreateBufferRegionARB(long hdc, int layerPlane, int type) {
		long __functionAddress = GL.getCapabilities().wglCreateBufferRegionARB;
		if ( CHECKS ) {
	...
	public static void wglDeleteBufferRegionARB(long region) {
		long __functionAddress = GL.getCapabilities().wglDeleteBufferRegionARB;
		if ( CHECKS ) {
			checkFunctionAddress(__functionAddress);
			checkPointer(region);
		}
	...
	public static int wglSaveBufferRegionARB(long region, int x, int y, int width, int height) {
		long __functionAddress = GL.getCapabilities().wglSaveBufferRegionARB;
		if ( CHECKS ) {
			checkFunctionAddress(__functionAddress);
			checkPointer(region);
		}
	...
	public static int wglRestoreBufferRegionARB(long region, int x, int y, int width, int height, int xSrc, int ySrc) {
		long __functionAddress = GL.getCapabilities().wglRestoreBufferRegionARB;
		if ( CHECKS ) {
			checkFunctionAddress(__functionAddress);
			checkPointer(region);
		}
