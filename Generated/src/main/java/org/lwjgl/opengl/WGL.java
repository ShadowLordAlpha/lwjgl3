...
public class WGL {
	...
	static boolean isAvailable(GLCapabilities caps) {
		return checkFunctions(
			caps.wglCreateContext, caps.wglCreateLayerContext, caps.wglCopyContext, caps.wglDeleteContext, caps.wglGetCurrentContext, caps.wglGetCurrentDC, 
			caps.wglGetProcAddress, caps.wglMakeCurrent, caps.wglShareLists, caps.wglUseFontBitmaps, caps.wglUseFontOutlines
		);
	}
	...
	public static long wglCreateContext(long hdc) {
		long __functionAddress = GL.getCapabilities().wglCreateContext;
		if ( CHECKS ) {
	...
	public static long wglCreateLayerContext(long hdc, int layerPlane) {
		long __functionAddress = GL.getCapabilities().wglCreateLayerContext;
		if ( CHECKS ) {
			checkFunctionAddress(__functionAddress);
			checkPointer(hdc);
		}
	...
	public static int wglCopyContext(long src, long dst, int mask) {
		long __functionAddress = GL.getCapabilities().wglCopyContext;
		if ( CHECKS ) {
			checkFunctionAddress(__functionAddress);
			checkPointer(src);
			checkPointer(dst);
	...
	public static int wglDeleteContext(long context) {
		long __functionAddress = GL.getCapabilities().wglDeleteContext;
		if ( CHECKS ) {
			checkFunctionAddress(__functionAddress);
	...
	public static long wglGetCurrentContext() {
		long __functionAddress = GL.getCapabilities().wglGetCurrentContext;
		if ( CHECKS )
			checkFunctionAddress(__functionAddress);
	...
	public static long wglGetCurrentDC() {
		long __functionAddress = GL.getCapabilities().wglGetCurrentDC;
		if ( CHECKS )
			checkFunctionAddress(__functionAddress);
		return callP(__functionAddress);
	}
	...
	public static long nwglGetProcAddress(long proc) {
		long __functionAddress = GL.getCapabilities().wglGetProcAddress;
		if ( CHECKS )
			checkFunctionAddress(__functionAddress);
	...
	public static long wglGetProcAddress(CharSequence proc) {
		...
		try {
			ByteBuffer procEncoded = memEncodeASCII(proc, true, stack);
	...
	public static int wglMakeCurrent(long hdc, long context) {
		long __functionAddress = GL.getCapabilities().wglMakeCurrent;
		if ( CHECKS )
			checkFunctionAddress(__functionAddress);
	...
	public static int wglShareLists(long context1, long context2) {
		long __functionAddress = GL.getCapabilities().wglShareLists;
		if ( CHECKS ) {
			checkFunctionAddress(__functionAddress);
	...
	public static int wglUseFontBitmaps(long hdc, int first, int count, int listBase) {
		long __functionAddress = GL.getCapabilities().wglUseFontBitmaps;
		if ( CHECKS ) {
			checkFunctionAddress(__functionAddress);
			checkPointer(hdc);
		}
	...
	public static int nwglUseFontOutlines(long hdc, int first, int count, int listBase, float deviation, float extrusion, int format, long glyphMetrics) {
		long __functionAddress = GL.getCapabilities().wglUseFontOutlines;
		if ( CHECKS ) {
			checkFunctionAddress(__functionAddress);
			checkPointer(hdc);
		}
