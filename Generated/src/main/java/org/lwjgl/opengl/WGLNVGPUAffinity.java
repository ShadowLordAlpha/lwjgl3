...
public class WGLNVGPUAffinity {
	...
	public static int nwglEnumGpusNV(int gpuIndex, long gpu) {
		long __functionAddress = GL.getCapabilities().wglEnumGpusNV;
		if ( CHECKS )
	...
	public static int nwglEnumGpuDevicesNV(long gpu, int deviceIndex, long gpuDevice) {
		long __functionAddress = GL.getCapabilities().wglEnumGpuDevicesNV;
		if ( CHECKS ) {
			checkFunctionAddress(__functionAddress);
			checkPointer(gpu);
		}
	...
	public static long nwglCreateAffinityDCNV(long gpuList) {
		long __functionAddress = GL.getCapabilities().wglCreateAffinityDCNV;
		if ( CHECKS )
			checkFunctionAddress(__functionAddress);
	...
	public static int nwglEnumGpusFromAffinityDCNV(long affinityDC, int gpuIndex, long gpu) {
		long __functionAddress = GL.getCapabilities().wglEnumGpusFromAffinityDCNV;
		if ( CHECKS ) {
			checkFunctionAddress(__functionAddress);
	...
	public static int wglDeleteDCNV(long hdc) {
		long __functionAddress = GL.getCapabilities().wglDeleteDCNV;
		if ( CHECKS ) {
			checkFunctionAddress(__functionAddress);
			checkPointer(hdc);
		}
