...
public class GLX11 {
	...
	public static long nglXQueryExtensionsString(long display, int screen) {
		long __functionAddress = GL.getCapabilities().glXQueryExtensionsString;
		if ( CHECKS ) {
	...
	public static long nglXGetClientString(long display, int name) {
		long __functionAddress = GL.getCapabilities().glXGetClientString;
		if ( CHECKS ) {
			checkFunctionAddress(__functionAddress);
			checkPointer(display);
		}
	...
	public static long nglXQueryServerString(long display, int screen, int name) {
		long __functionAddress = GL.getCapabilities().glXQueryServerString;
		if ( CHECKS ) {
			checkFunctionAddress(__functionAddress);
			checkPointer(display);
		}
