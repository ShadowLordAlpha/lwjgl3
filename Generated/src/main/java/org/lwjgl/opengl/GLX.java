...
public class GLX {
	...
	static boolean isAvailable(GLCapabilities caps) {
		return checkFunctions(
			caps.glXQueryExtension, caps.glXQueryVersion, caps.glXGetConfig, caps.glXChooseVisual, caps.glXCreateContext, caps.glXMakeCurrent, 
			caps.glXCopyContext, caps.glXIsDirect, caps.glXDestroyContext, caps.glXGetCurrentContext, caps.glXGetCurrentDrawable, caps.glXWaitGL, caps.glXWaitX, 
			caps.glXSwapBuffers, caps.glXUseXFont, caps.glXCreateGLXPixmap, caps.glXDestroyGLXPixmap
		);
	}
	...
	public static int nglXQueryExtension(long display, long error_base, long event_base) {
		long __functionAddress = GL.getCapabilities().glXQueryExtension;
		if ( CHECKS ) {
	...
	public static int nglXQueryVersion(long display, long major, long minor) {
		long __functionAddress = GL.getCapabilities().glXQueryVersion;
		if ( CHECKS ) {
			checkFunctionAddress(__functionAddress);
			checkPointer(display);
		}
	...
	public static int nglXGetConfig(long display, long visual, int attribute, long value) {
		long __functionAddress = GL.getCapabilities().glXGetConfig;
		if ( CHECKS ) {
			checkFunctionAddress(__functionAddress);
			checkPointer(display);
	...
	public static long nglXChooseVisual(long display, int screen, long attrib_list) {
		long __functionAddress = GL.getCapabilities().glXChooseVisual;
		if ( CHECKS ) {
			checkFunctionAddress(__functionAddress);
			checkPointer(display);
		}
	...
	public static long nglXCreateContext(long display, long visual, long share_list, int direct) {
		long __functionAddress = GL.getCapabilities().glXCreateContext;
		if ( CHECKS ) {
			checkFunctionAddress(__functionAddress);
			checkPointer(display);
			XVisualInfo.validate(visual);
	...
	public static int glXMakeCurrent(long display, long draw, long ctx) {
		long __functionAddress = GL.getCapabilities().glXMakeCurrent;
		if ( CHECKS ) {
			checkFunctionAddress(__functionAddress);
			checkPointer(display);
		}
	...
	public static void glXCopyContext(long display, long source, long dest, long mask) {
		long __functionAddress = GL.getCapabilities().glXCopyContext;
		if ( CHECKS ) {
			checkFunctionAddress(__functionAddress);
			checkPointer(display);
			checkPointer(source);
	...
	public static int glXIsDirect(long display, long ctx) {
		long __functionAddress = GL.getCapabilities().glXIsDirect;
		if ( CHECKS ) {
			checkFunctionAddress(__functionAddress);
			checkPointer(display);
			checkPointer(ctx);
	...
	public static void glXDestroyContext(long display, long ctx) {
		long __functionAddress = GL.getCapabilities().glXDestroyContext;
		if ( CHECKS ) {
			checkFunctionAddress(__functionAddress);
			checkPointer(display);
			checkPointer(ctx);
	...
	public static long glXGetCurrentContext() {
		long __functionAddress = GL.getCapabilities().glXGetCurrentContext;
		if ( CHECKS )
			checkFunctionAddress(__functionAddress);
	...
	public static long glXGetCurrentDrawable() {
		long __functionAddress = GL.getCapabilities().glXGetCurrentDrawable;
		if ( CHECKS )
			checkFunctionAddress(__functionAddress);
		return callP(__functionAddress);
	}
	...
	public static void glXWaitGL() {
		long __functionAddress = GL.getCapabilities().glXWaitGL;
		if ( CHECKS )
			checkFunctionAddress(__functionAddress);
	...
	public static void glXWaitX() {
		long __functionAddress = GL.getCapabilities().glXWaitX;
		if ( CHECKS )
			checkFunctionAddress(__functionAddress);
		callV(__functionAddress);
	}
	...
	public static void glXSwapBuffers(long display, long draw) {
		long __functionAddress = GL.getCapabilities().glXSwapBuffers;
		if ( CHECKS ) {
			checkFunctionAddress(__functionAddress);
			checkPointer(display);
			checkPointer(draw);
	...
	public static void glXUseXFont(long font, int first, int count, int list_base) {
		long __functionAddress = GL.getCapabilities().glXUseXFont;
		if ( CHECKS )
			checkFunctionAddress(__functionAddress);
	...
	public static long nglXCreateGLXPixmap(long display, long visual, long pixmap) {
		long __functionAddress = GL.getCapabilities().glXCreateGLXPixmap;
		if ( CHECKS ) {
			checkFunctionAddress(__functionAddress);
			checkPointer(display);
			XVisualInfo.validate(visual);
	...
	public static void glXDestroyGLXPixmap(long display, long pixmap) {
		long __functionAddress = GL.getCapabilities().glXDestroyGLXPixmap;
		if ( CHECKS ) {
			checkFunctionAddress(__functionAddress);
			checkPointer(display);
			checkPointer(pixmap);
