...
public class WGLNVDXInterop {
	...
	public static int wglDXSetResourceShareHandleNV(long dxObject, long shareHandle) {
		long __functionAddress = GL.getCapabilities().wglDXSetResourceShareHandleNV;
		if ( CHECKS ) {
	...
	public static long wglDXOpenDeviceNV(long dxDevice) {
		long __functionAddress = GL.getCapabilities().wglDXOpenDeviceNV;
		if ( CHECKS ) {
			checkFunctionAddress(__functionAddress);
	...
	public static int wglDXCloseDeviceNV(long device) {
		long __functionAddress = GL.getCapabilities().wglDXCloseDeviceNV;
		if ( CHECKS ) {
			checkFunctionAddress(__functionAddress);
			checkPointer(device);
		}
	...
	public static long wglDXRegisterObjectNV(long device, long dxResource, int name, int type, int access) {
		long __functionAddress = GL.getCapabilities().wglDXRegisterObjectNV;
		if ( CHECKS ) {
			checkFunctionAddress(__functionAddress);
			checkPointer(device);
	...
	public static int wglDXUnregisterObjectNV(long device, long object) {
		long __functionAddress = GL.getCapabilities().wglDXUnregisterObjectNV;
		if ( CHECKS ) {
			checkFunctionAddress(__functionAddress);
			checkPointer(device);
			checkPointer(object);
	...
	public static int wglDXObjectAccessNV(long object, int access) {
		long __functionAddress = GL.getCapabilities().wglDXObjectAccessNV;
		if ( CHECKS ) {
			checkFunctionAddress(__functionAddress);
			checkPointer(object);
		}
	...
	public static int nwglDXLockObjectsNV(long device, int count, long objects) {
		long __functionAddress = GL.getCapabilities().wglDXLockObjectsNV;
		if ( CHECKS ) {
			checkFunctionAddress(__functionAddress);
			checkPointer(device);
		}
	...
	public static int nwglDXUnlockObjectsNV(long device, int count, long objects) {
		long __functionAddress = GL.getCapabilities().wglDXUnlockObjectsNV;
		if ( CHECKS ) {
			checkFunctionAddress(__functionAddress);
			checkPointer(device);
		}
