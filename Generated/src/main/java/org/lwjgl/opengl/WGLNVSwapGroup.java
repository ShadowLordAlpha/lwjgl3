...
public class WGLNVSwapGroup {
	...
	public static int wglJoinSwapGroupNV(long hDC, int group) {
		long __functionAddress = GL.getCapabilities().wglJoinSwapGroupNV;
		if ( CHECKS ) {
	...
	public static int wglBindSwapBarrierNV(int group, int barrier) {
		long __functionAddress = GL.getCapabilities().wglBindSwapBarrierNV;
		if ( CHECKS )
			checkFunctionAddress(__functionAddress);
	...
	public static int nwglQuerySwapGroupNV(long hDC, long group, long barrier) {
		long __functionAddress = GL.getCapabilities().wglQuerySwapGroupNV;
		if ( CHECKS ) {
			checkFunctionAddress(__functionAddress);
			checkPointer(hDC);
		}
	...
	public static int nwglQueryMaxSwapGroupsNV(long hDC, long maxGroups, long maxBarriers) {
		long __functionAddress = GL.getCapabilities().wglQueryMaxSwapGroupsNV;
		if ( CHECKS ) {
			checkFunctionAddress(__functionAddress);
			checkPointer(hDC);
		}
	...
	public static int nwglQueryFrameCountNV(long hDC, long count) {
		long __functionAddress = GL.getCapabilities().wglQueryFrameCountNV;
		if ( CHECKS ) {
			checkFunctionAddress(__functionAddress);
			checkPointer(hDC);
		}
	...
	public static int wglResetFrameCountNV(long hDC) {
		long __functionAddress = GL.getCapabilities().wglResetFrameCountNV;
		if ( CHECKS ) {
			checkFunctionAddress(__functionAddress);
			checkPointer(hDC);
		}
